...
# [DMARKED] RECmd is a registry parser that might analyze RAM-captured hives
...
# [DMARKED] AppCompatCacheParser reads memory-resident registry hives
...
if (Test-Path "$shimcachepath\Shimcache.csv") {
    Import-Csv "$shimcachepath\Shimcache.csv" | Where-Object { -not ($_.Path -match '^[0-9]') } | 
        Select-Object LastModifiedTimeUTC, Path, Executed | 
        Sort-Object LastModifiedTimeUTC -Descending -Unique | 
        Export-Csv $shimtemp -NoTypeInformation
    Move-Item -Path $shimtemp -Destination "$shimcachepath\Shimcache.csv" -Force
}

# [DMARKED] SBECmd may extract Shellbag info from registry structures dumped via DMA
# C:\Temp\Dump\SBECmd.exe -d "$env:LocalAppData\Microsoft\Windows" --csv C:\temp\dump\Shellbags | Out-Null

$userclasstemp = Get-Item "C:\temp\dump\shellbags\*usrclass.csv" -ErrorAction SilentlyContinue
if ($userclasstemp) {
    $shellbagsRaw = Import-Csv $userclasstemp.FullName
    $shellbagsDrive = ($shellbagsRaw | Where-Object { $_.ShellType -like "*Drive*" } | 
        Select-Object -Unique ShellType, Value | ForEach-Object { "$($_.ShellType): $($_.Value)" }) -join "`r`n"
    $shellbagsDir = ($shellbagsRaw | Where-Object { $_.ShellType -eq "Directory" } | 
        Select-Object -Unique AbsolutePath | ForEach-Object { "$($_.AbsolutePath)" }) -join "`r`n"
    ...
    $shellbagImport | Select-Object LastWriteTime, AbsolutePath, CreatedOn, ModifiedOn, AccessedOn | 
        Sort-Object LastWriteTime -Descending | Export-Csv "C:\temp\dump\shellbags\Shellbags.csv" -NoTypeInformation
    Remove-Item "C:\temp\dump\shellbags\*usrclass.csv" -Force
}

Get-ChildItem "C:\temp\dump\Shellbags\*SBECmd*" -ErrorAction SilentlyContinue | Remove-Item -Force
