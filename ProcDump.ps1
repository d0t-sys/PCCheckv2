...
# Entfernte DMA-nahe Komponenten für Sicherheit
# hollows_hunter.exe wurde deaktiviert
...
Get-Content explorer.txt | Where-Object { $_ -match "^[A-Za-z]:\\.*\.dll$" } | Sort-Object -Unique -Descending | Out-File "$procpath\DLL.txt"
...
$dps2 = $predps2 | Where-Object { $_ -match ('!!' + ($dps2grouped -join '|') + '!') } -join "`n"
...
Get-Content explorer.txt | Where-Object { $_ -match "file:///" } | ForEach-Object { $_ -replace "file:///", "" } | Out-File "$procpath\Files_Visited.txt"

Get-Content explorer.txt, diagtrack.txt | Where-Object { $_ -match "HarddiskVolume" } | ForEach-Object { if ($_ -match "HarddiskVolume(\d+)") { [PSCustomObject]@{ Line = $_; Number = $matches[1] } } } | Group-Object Number | Sort-Object Count | ForEach-Object { $_.Group.Line } | Select-Object -Unique | Out-File "$procpath\Harddiskvolumes.txt"

Get-Content explorer.txt | Where-Object { $_ -match "[A-Z]:\\.*[^\x00-\x7F].*\.exe" } | Out-File "$procpath\Invisible_Chars.txt"
...
$modext = "Possible Modification of Extensions in DPS`n$modext1 `nPossible Modification of Extensions in Diagtrack`n$modext2"
$modext | Out-File "$procpath\Modified_Extensions.txt"

Get-Content explorer.txt | Where-Object { $_ -match "pcaclient" } | ForEach-Object { if ($_ -match "[A-Z]:\\.*?\.exe") { $matches[0] } } | Sort-Object -Unique -Descending | Out-File "$procpath\PcaClient.txt"
...
$pca2 | ForEach-Object { if ($_ -match "[A-Z]:\\.*?\.exe") { $matches[0] } } | Sort-Object -Unique -Descending | Out-File "$procpath\Pca_Extended.txt"

Get-Content explorer.txt, pcasvc.txt, diagtrack.txt | Where-Object { $_ -match "^[a-zA-Z0-9_-]+\.(rar|zip|7z)$" } | Out-File "$procpath\Compressed_Processes.txt"
Get-Content explorer.txt | Where-Object { $_ -match "^\b(?!C:)[A-Z]:\\.*" } | Sort-Object -Unique -Descending | Out-File "$procpath\Drive_Executables.txt"
Get-Content explorer.txt | Where-Object { $_ -match "^[a-zA-Z0-9_-]+\.(bat|ps1)$" } | Sort-Object -Unique -Descending | Out-File "$procpath\Scripts.txt"
Get-Content explorer.txt | Where-Object { $_ -match "Local\\Temp.*\.exe" } | Sort-Object -Unique -Descending | Out-File "$procpath\Compressed_Temp.txt"

if (Test-Path "C:\windows\appcompat\pca\PcaAppLaunchDic.txt") { Copy-Item "C:\windows\appcompat\pca\PcaAppLaunchDic.txt" -Destination "$procpathraw" }
Get-Content "$procpathraw\PcaAppLaunchDic.txt" | ForEach-Object { ($_ -replace '\|.*') } | Where-Object { $_ -match '^[A-Za-z]:\\' } | Out-File "$procpath\Pca_AppLauncher.txt"

$sUptime | Out-File "$procpath\Uptime.txt"

Move-Item -Path "$procpath\*.txt" -Destination "$procpathfilt"

# Deaktiviert: potenziell speichernahe Analyse
